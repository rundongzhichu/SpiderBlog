...
var user = document.getElementById("user")[0] // 获取用户名
...
function login() {
    if (onoff) {
        let request = new XMLHttpRequest()
        let url = ""
        request.open("post", url, true) // true 是否异步，如果是状态改变时调用onreadystatechange 属性指定回调函数
        let data = new FormData()
        data.append("user", user.value)
        data.append("passwd", passwd.value) // data 为请求数据
        request.onreadystatechange = function() {
            if (this.readyState == 4) {
